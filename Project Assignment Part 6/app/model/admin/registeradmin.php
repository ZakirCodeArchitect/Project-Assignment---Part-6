...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM admin WHERE username='$userId' ";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Username already exists, display alert message
        echo "<script>alert('Admin already exists.'); window.location.href = 'loginCR.html';</script>";
    } else {
        // Insert the new user into the database
        $sql = "INSERT INTO admin (username, Password) VALUES ('$userId', '$password')";
